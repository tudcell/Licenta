"""
Wallet Module - Managing user keys and identity.
"""
...
from src.domain.crypto.digital_signature import DigitalSignature, KeyPair
from src.domain.transaction import Transaction, TransactionFactory
...
@dataclass
class Wallet:
    """Digital wallet for managing identity and signing transactions."""
    ...
    def load(cls, filepath: str) -> 'Wallet':
        ...
        wallet = cls(name=data["name"], _key_pair=KeyPair(private_key=private_key, public_key=public_key))
...
class WalletManager:
    ...
    def create_wallet(self, name: str, metadata: Dict[str, Any] = None, persist_private_key: bool = True) -> Wallet:
        ...
        wallet.save(filepath, include_private_key=persist_private_key)
        return wallet
