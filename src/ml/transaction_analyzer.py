...
class TransactionAnalyzer:
    ...
    def _is_clean_training_candidate(self, transaction: Transaction) -> bool:
        ...
        if risk_level in {'high', 'critical'}:
            return False
        if transaction.transaction_type in {TransactionType.LOGIN_FAILED, TransactionType.ACCESS_DENIED, TransactionType.DATA_DELETE}:
            return False
        return True

    def _get_clean_training_transactions(self, transactions: Optional[List[Transaction]] = None) -> List[Transaction]:
        source = transactions or self._historical_transactions
    ...
    def add_transaction(self, transaction: Transaction) -> AuditReport:
        ...
        if self.detector.is_fitted and signature_valid:
            ...
            flagged_for_review = anomaly_result.is_anomaly
    ...
    def mine_and_analyze(self) -> Optional[Dict[str, Any]]:
        ...
        for tx in new_block.transactions:
            ...
            existing_alert = next((a for a in self.alerts if a.transaction_id == tx.transaction_id), None)
            if existing_alert:
    ...
    def analyze_transaction(self, transaction_id: str) -> Optional[AuditReport]:
        ...
        if self.detector.is_fitted:
            ...
            tx_index = next((i for i, t in enumerate(all_transactions) if t.transaction_id == transaction_id), -1)
